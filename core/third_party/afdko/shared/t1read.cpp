...
#include "t1read_keys.h"  // NOLINT(build/include)
#undef DCL_KEY
};
...
static void encAdd(t1rCtx h, Char *chr, unsigned long code) {
    ...
    if (strcmp(chr->gname.ptr, ".notdef") == 0)
        ...
        abfEncoding *nw = h->encfree;
        if (nw == NULL)
            /* Get node from heap */
            nw = (abfEncoding*)memNew(h, sizeof(abfEncoding));
        else
...
t1rCtx t1rNew(ctlMemoryCallbacks *mem_cb, ctlStreamCallbacks *stm_cb,
    ...
    /* Allocate context */
    h = (t1rCtx) mem_cb->manage(mem_cb, NULL, sizeof(struct t1rCtx_));
    if (h == NULL)
        return NULL;
...
static void findToken(t1rCtx h, const char *value) {
...
static void saveCstr(t1rCtx h, long length, char *value, Char *chr,
                     int update) {
...
static void prepMMData(t1rCtx h) {
    ...
    coords[0] = '_';
    p = &coords[1];
    int coordslen = sizeof(coords)-1;
    for (i = 0; i < nAxes; i++) {
        snprintf(p, coordslen, "%d_", (int)RND(h->fd->aux.UDV[i]));
        int l = strlen(p);
        p += l;
        coordslen -= l;
    }
    ...
    if (lenCoords + lenFontName + 1 > sizeof(buf))
        ...
        snprintf(buf, sizeof(buf), "%s%s", FontName, coords);
...
static long parseInt(t1rCtx h, int kKey) {
    ...
    switch (token->type) {
        ...
        case pstArray:
        case pstProcedure: {
                char *pp = copyArrayToken(h, token);
                return (long)RND(parseBlend(h, kKey, &pp));
            }
        break;
        default:
            badKeyValue(h, kKey);
...
static void parseErodeProc(t1rCtx h, abfPrivateDict *priv) {
    int i;
    pstToken *token = getToken(h);
...
static void parseOrigFontType(t1rCtx h, abfTopDict *top) {
    ...
    if (token->type == pstLiteral) {
        ...
        unsigned int i;
        long length;
        const char *value = pstConvLiteral(h->pst, token, &length);
    ...
    switch (keyId) {
            ...
            if (h->key.seen[kFamilyBlues] && !h->key.seen[kWeightVector])
               ...
               priv->FamilyBlues.cnt =
                parseNumArray(h, kFamilyBlues, 0, 14,
                              priv->FamilyBlues.array, 1, 1);
            break;
        case kFamilyOtherBlues:
           priv->FamilyOtherBlues.cnt =
                parseNumArray(h, kFamilyOtherBlues, 0, 10,
                              priv->FamilyOtherBlues.array, 1, 1);
            break;
        case kBlueScale:
           priv->BlueScale = parseNum(h, kBlueScale, 0);
            break;
        case kBlueShift:
           priv->BlueShift = parseNum(h, kBlueShift, 0);
            break;
        case kBlueFuzz:
           priv->BlueFuzz = parseNum(h, kBlueFuzz, 1);
            break;
        case kStdHW:
            (void)parseNumArray(h, kStdHW, 0, 1, &priv->StdHW, 1, 1);
            break;
        case kStdVW:
            (void)parseNumArray(h, kStdVW, 0, 1, &priv->StdVW, 1, 1);
            break;
        case kStemSnapH:
           priv->StemSnapH.cnt =
                parseNumArray(h, kStemSnapH, 0, 12,
                              priv->StemSnapH.array, 1, 1);
            break;
        case kStemSnapV:
           priv->StemSnapV.cnt =
                parseNumArray(h, kStemSnapV, 0, 12,
                              priv->StemSnapV.array, 1, 1);
            break;
        case kForceBold:
           priv->ForceBold = parseBool(h, kForceBold);
            break;
        ...
        case kRndStemUp:
           ...
           priv->LanguageGroup = 1;
            break;
        case kLanguageGroup:
           priv->LanguageGroup = parseInt(h, kLanguageGroup);
            break;
        case kExpansionFactor:
           priv->ExpansionFactor = parseNum(h, kExpansionFactor, 1);
            break;
        case kinitialRandomSeed:
           priv->initialRandomSeed = parseNum(h, kinitialRandomSeed, 0);
            break;
        case kSubrs:
            readSubrs(h);
            break;
        case kErode:
            parseErodeProc(h, priv);
            break;
        case kRD:
            checkRDType(h);
            break;
...
/* Process operator. */
static void doOperator(t1rCtx h, pstToken *op) {
    if (pstMatch(h->pst, op, "currentfile")) {
        pstToken *token;
        int result = 0;
        token = getToken(h);
        ...
        if (result)
            pstFatal(h, result);
               ...
               pstMatch(h->pst, op, "FontDirectory")) {
...
static void saveStdSubrs(t1rCtx h) {
    ...
    for (i = 0; i < h->FDArray.cnt; i++) {
        ...
        dnaSET_CNT(fd->subrs.offset, kSubrcnt);
...
static void cidSaveChar(t1rCtx h, long len, const char *cstr, long cid) {
    unsigned char iFD;
    ...
    /* Save charstring */
    saveCstr(h, len, (char *)cstr, chr, 1);
}
...
parse_hex_fmt:
    for (;;) {
        ...
        if (tokenType == pstASCII85) {
            cstr = token->value + 2;
            len = convASCII85Cstr(h, (char *)cstr);
        } else {
            cstr = token->value + 1;
            len = convASCIIHexCstr(h, token->length - 2, (char *)cstr);
        }
        if (len == 0)
            goto bad_char;
...
parse_bin_fmt:
    for (;;) {
        int result = pstRead(h->pst, len + 1, (char **)&cstr);
        if (result)
            pstFatal(h, result);
...
int t1rGetGlyphByName(t1rCtx h, const char *gname, abfGlyphCallbacks *glyph_cb) {
    size_t index;
