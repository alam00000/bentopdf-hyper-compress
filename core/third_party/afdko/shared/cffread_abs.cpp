...
static const char *stdstrs[] = {
#include "stdstr1.h"
};

/* Glyph names for Standard Apple Glyph Ordering */
static const char *applestd[258] = {
#include "applestd.h"
};
...
static const char *sid2str(cfrCtx h, long sid) {
    if (sid < 0) {
        // noop
...
static const char *savePostScript(cfrCtx h, const char *str) {
    int32_t value;
    int n;
    const char *p;
    char *q;

    /* The strings passed here are a mix of const values from the
     * file included above and char *s extracted from the file.
     * Only the latter have parts like /FSType, and we need to
     * modify those. So we explicitly cast away const here when
     * we've determined that the string can and should be modified.
     */
    p = strstr(str, "/FSType");
    if (p != NULL) {
        // Extract FSType from string
        n = -1;
        q = (char *) p + sizeof("/FSType") - 1;
    ...
    if (p != NULL) {
        ...
        n = -1;
        q = (char *) p + sizeof("/OrigFontType") - 1;
...
static const char *post2GetName(cfrCtx h, SID gid) {
...
static void buildGIDNames(cfrCtx h) {
    ...
    /* Read string data */
    blength = numGlyphs * 9; /* 3 for 'gid', 5 for GID, 1 for null termination. */
    dnaSET_CNT(h->post.fmt2.buf, blength + 1);
    /* Build C strings array */
    dnaSET_CNT(h->post.fmt2.strings, numGlyphs);
    p = h->post.fmt2.buf.array;
    snprintf(p, blength, ".notdef");
    h->post.fmt2.strings.array[0] = p;
    slength = (long)strlen(p);
    p += slength + 1;
    blength -= slength + 1;
    for (i = 1; i < numGlyphs; i++) {
        h->post.fmt2.strings.array[i] = p;
        snprintf(p, blength, "gid%05ld", i);
        slength = (long)strlen(p);
        p += slength + 1;
        blength -= slength + 1;
    }
...
static void addString(cfrCtx h, strPtrArray *ptrs, abfString *abfStr, const char *str) {
...
const char *cfrErrStr(int err_code) {
