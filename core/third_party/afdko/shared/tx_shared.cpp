...
static void *mem_manage(ctlMemoryCallbacks *cb, void *old, size_t size) {
    if (size > 0) {
        txCtx h = (txCtx) cb->ctx;
        if (h->failmem.iCall++ == h->failmem.iFail) {
            /* Simulate memory allocation failure */
            h->logger->log(sWARNING, "mem_manage() failed on call %ld.",
                           h->failmem.iCall - 1L);
            return NULL;
        } else if (old == NULL)
            return sMemNew(size); /* size != 0, old == NULL */
        else
            return sMemResize(old, size); /* size != 0, old != NULL */
    } else {
        if (old == NULL)
            return NULL; /* size == 0, old == NULL */
        else {
            sMemFree(old); /* size == 0, old != NULL */
            return NULL;
        }
    }
}
...
static void tmpSet(Stream *s, const char *filename) {
    s->type = stm_Tmp;
    s->flags = 0;
    s->filename = filename;
    s->fp = NULL;
...
static Stream *tmp_open(txCtx h, Stream *s) {
    s->buf = (char *)sMemNew(TMPSIZE + BUFSIZ);
...
/* Write to tmp stream. */
static size_t tmp_write(Stream *s, size_t count, const char *ptr) {
    if (s->pos < TMPSIZE) {
        size_t length;
        /* Writing to buffer */
        length = TMPSIZE - s->pos;
    ...
    switch (id) {
        ...
        case UFO_SRC_STREAM_ID: {
            if (cb->clientFileName != NULL) {
                ...
                snprintf(buffer, sizeof(buffer), "%s/%s", h->file.src,
                         cb->clientFileName);
                s = &h->src.stm;
                s->type = stm_SrcUFO;
        ...
        case UFW_DST_STREAM_ID: {
            if (cb->clientFileName != NULL) {
                char buffer[FILENAME_MAX];
                snprintf(buffer, sizeof(buffer), "%s/%s", h->file.dst,
                         cb->clientFileName);
                s = &h->dst.stm;
...
static size_t stm_xml_read(ctlStreamCallbacks *cb, void *stream,
                           xmlDocPtr *doc) {
    int res;
    int readAmt = 0;
...
static int stm_close(ctlStreamCallbacks *cb, void *stream) {
    ...
    Stream *s = (Stream *)stream;
    if (s->type == stm_Tmp)
        return tmp_close((txCtx)cb->direct_ctx, s);
...
static void stmSet(Stream *s, int type, const char *filename, char *buf) {
    s->type = type;
    s->flags = 0;
    s->filename = filename;
    s->fp = NULL;
...
void dstFileSetName(txCtx h, const char *filename) {
    if (h->file.dd != NULL)
        snprintf(h->file.dst, sizeof(h->file.dst),
                 "%s/%s", h->file.dd, filename);
    else
        STRCPY_S(h->file.dst, sizeof(h->file.dst), filename);
}
...
static void dstFileSetAutoName(txCtx h, abfTopDict *top) {
    ...
    if (h->flags & AUTO_FILE_FROM_FILE) {
        ...
        STRCPY_S(buf, sizeof(buf), (p == NULL) ? h->file.src : p + 1);
        p = strrchr(buf, '.');
        if (p != NULL)
            *p = '\0';
        filename = buf;
    ...
    if (h->file.dd != NULL)
        snprintf(h->file.dst, sizeof(h->file.dst),
                 "%s/%s.%s", h->file.dd, filename, h->modename);
    else
        snprintf(h->file.dst, sizeof(h->file.dst),
                 "%s.%s", filename, h->modename);
}
...
static void cff_EndSet(txCtx h) {
    ...
    if (h->ext == NULL) {
        if (abfFree(h->abf.ctx))
            fatal(h, NULL);
    } else if (h->ext->cff_EndSetCB != NULL) {
        (*h->ext->cff_EndSetCB)(h->ext);
    }
}
...
static void cff_SetMode(txCtx h) {
    ...
    } else if (h->ext->cff_SetModeCB != NULL) {
        (*h->ext->cff_SetModeCB)(h->ext);
    }
    h->mode = mode_cff;
}
...
static void makeSubsetArgList(txCtx h) {
    ...
    for (i = 1; i <= h->subset.glyphs.cnt; i++) {
        ...
        if (last + 1 != curr) {
            ...
            if (first == last)
                snprintf(buf, sizeof(buf), "%hu", last);
            else
                snprintf(buf, sizeof(buf), "%hu-%hu", first, last);
            long slen = strlen(buf)+1;
            STRCPY_S(dnaEXTEND(h->subset.args, slen), slen, buf);
            first = curr;
            rangecnt++;
        }
        last = curr;
...
/* Lookup glyph. */
static void lookupGlyph(txCtx h, const void *key, ctuMatchFunc match) {
    size_t index;
...
/* Select subset glyph. */
static void selectGlyph(txCtx h, int type, unsigned short id, const char *gname) {
    switch (type) {
        case sel_by_tag:
...
static unsigned short mapName2UV(txCtx h, const char *gname,
...
static int t1_GlyphBeg(abfGlyphCallbacks *cb, abfGlyphInfo *info) {
    ...
    if (h->ext == NULL || h->ext->flags & EXT_SKIP_SEEN) {
    ...
    if (h->t1w.fd == -1) {
        ...
        if (h->ext != NULL && !(h->ext->flags & EXT_NO_SKIP_NOTDEF)) {
            /* Handle .notdef glyph */
            if (info->flags & ABF_GLYPH_SEEN)
                return ABF_SKIP_RET; /* Already in subset */
        }
    ...
    if (info->gname.ptr != NULL) {
        STRCPY_S(gname, sizeof(gname), info->gname.ptr);
    } else {
        if (info->cid == 0)
            STRCPY_S(gname, sizeof(gname), ".notdef");
        else
            snprintf(gname, sizeof(gname), "cid%hu", info->cid);
    }
    ...
    char *gnptr = &h->t1w.gnames.array[h->t1w.gnames.cnt];
    info->gname.impl = h->t1w.gnames.cnt;  // hold current char index, to make renumbering this easier!
    STRCPY_S(gnptr, nameLen, gname);
    info->gname.ptr = gnptr;
...
static void t1_BegFont(txCtx h, abfTopDict *top) {
    ...
    } else if (h->ext->t1_BegFontCB != NULL) {
        (*h->ext->t1_BegFontCB)(h->ext);
    }
...
static void copyFile(txCtx h, size_t length,
                     FILE *src, const char *srcfile,
                     FILE *dst, const char *dstfile) {
    char buf[BUFSIZ];
    size_t cnt;
...
static void writePFB(txCtx h, FILE *font, const char *fontfile,
                     long begBinary, long begTrailer, long endTrailer) {
    const char *tmpfil = "(t1w) reformat tmpfil";
    FILE *tmp = tmpfile();
    if (tmp == NULL)
        fileError(h, tmpfil);
...
static void writeLWFN(txCtx h, FILE *font, const char *fontfile,
                      long begBinary, long begTrailer, long endTrailer) {
    enum {
    ...
    const char *tmpfil = "(t1w) reformat tmpfile";
    FILE *tmp = tmpfile();
    if (tmp == NULL)
        fileError(h, tmpfil);
...
static void t1_Reformat(txCtx h) {
    ...
    const char *fontfile = h->dst.stm.filename;
...
//        }
    ...
    if (h->ext == NULL) {
        ...
        if (h->ext->t1_EndFontCB != NULL)
            (*h->ext->t1_EndFontCB)(h->ext);
        else if (t1wEndFont(h->t1w.ctx, h->top))
            fatal(h, NULL);
    }
...
static void t1_SetMode(txCtx h) {
    ...
    */
    ...
    } else if (h->ext->t1_SetModeCB != NULL) {
       (*h->ext->t1_SetModeCB)(h->ext);
    } else {
        /* Initialize glyph callbacks */
        h->cb.glyph = t1wGlyphCallbacks;
        h->cb.glyph.direct_ctx = h->t1w.ctx;
...
static void mkdir_tx(txCtx h, const char *dirPath) {
    int dirErr;
#ifdef _WIN32
    ...
    if (dirErr != 0)
        fatal(h, "Failed to create directory '%s'.", dirPath);
}
...
static void ufw_BegFont(txCtx h, abfTopDict *top) {
    ...
    if (statErrNo == 0) {
        fatal(h, "Destination UFO font already exists:  %s.", h->dst.stm.filename);
    } else {
        char buffer[FILENAME_MAX];
        mkdir_tx(h, h->dst.stm.filename);
        if (h->ufr.altLayerDir != NULL)
            snprintf(buffer, sizeof(buffer),
                     "%s/%s", h->file.dst, h->ufr.altLayerDir);
        else
            snprintf(buffer, sizeof(buffer),
                     "%s/%s", h->file.dst, "glyphs");
        mkdir_tx(h, buffer);
    }
...
/* Title flowed text. */
static void flowTitle(txCtx h, const char *title) {
    flowBeg(h);
...
/* Add flowed text. */
static void flowAdd(txCtx h, const char *fmt, ...) {
    va_list ap;
...
/* Break flowed text. */
static void flowBreak(txCtx h, const char *fmt, ...) {
    va_list ap;
    va_start(ap, fmt);
...
static void flowArg(txCtx h, const char *fmt, ...) {
    va_list ap;
    if (h->dcf.flags & DCF_SaveStemCnt)
        return;
    va_start(ap, fmt);
...
static void flowOp(txCtx h, const char *fmt, ...) {
    va_list ap;
    if (h->dcf.flags & DCF_SaveStemCnt)
        return;
    va_start(ap, fmt);
...
/* Dump table tag line. */
static void dumpTagLine(txCtx h, const char *title, const ctlRegion *region) {
    if (h->dcf.level < 1) {
        static char dots[] = " ................";
...
/* Dump CFF INDEX. */
static void dumpINDEX(txCtx h, const char *title, const ctlRegion *region,
...
static void flowCommand(txCtx h, const char *opname) {
    flowStack(h);
    flowOp(h, opname);
}
            ...
            case t2_vsindex: {
                ...
                if (h->dcf.varRegionInfo.cnt > 0) {
                    ...
                    if (vsIndex >= h->dcf.varRegionInfo.cnt) {
                        h->logger->msg(sWARNING, "invalid vsindex");
                    } else {
            ...
            case tx_callsubr:
                if (h->dcf.flags & DCF_Flatten) {
                    ...
                    if (h->dcf.subrDepth > TX_MAX_SUBR_DEPTH) {
                        fatal(h, "subr depth: %d", h->dcf.subrDepth);
                    }
    ...
    for (;;) {
        ...
        switch (c) {
            ...
            case 'a':
                ...
                /* Fall through */
            default:
                h->logger->log(sWARNING, "option -T invalid selector '%c' (ignored)", c);
        }
    }
}
...
/* Make random subset. */
static void makeRandSubset(txCtx h, const char *opt, char *arg) {
    long i;
    char *p;
    ...
    for (i = 0; i < h->subset.glyphs.cnt - 1; i++) {
        long j = randrange(h, h->subset.glyphs.cnt - i);
...
void prepSubset(txCtx h) {
    ...
    if ((h->mode == mode_cff || h->mode == mode_t1 || h->mode == mode_svg || h->mode == mode_ufow) && (h->flags & SHOW_NAMES)) {
        ...
        /* Print subset */
    h->logger->msg(sINFO,   "--- subset:");
    h->logger->log(sINDENT, "SRC font   %s", h->src.stm.filename);
    h->logger->log(sINDENT, "SRC glyphs %ld", h->top->sup.nGlyphs);
    h->logger->log(sINDENT, "DST font   %s", h->dst.stm.filename);
    h->logger->log(sINDENT, "DST glyphs %ld", h->subset.glyphs.cnt);
    std::string smsg;
        p = "";
        q = h->arg.g.substrs;
        for (i = 0; i < h->arg.g.cnt; i++) {
...
void buildFontList(txCtx h) {
    ...
    if (h->src.stm.fp == NULL) {
        ...
        snprintf(tempFileName, sizeof(tempFileName),
                 "%s/glyphs/contents.plist", h->file.src);
...
void setMode(txCtx h, int mode) {
    ...
    h->file.sd = NULL;
    h->file.dd = NULL;
    STRCPY_S(h->file.src, sizeof(h->file.src), "-");
    STRCPY_S(h->file.dst, sizeof(h->file.dst), "-");
...
static void usage(txCtx h) {
    static const char *text[] = {
#include "tx_help/tx_usage.h"
    };
    if (h->ext != NULL && h->ext->usage != NULL)
        (*h->ext->usage)(h->ext);
    else
        printText(ARRAY_LEN(text), text);
}
...
static void help(txCtx h) {
    static const char *text[] = {
#include "tx_help/tx_help.h"
    };
    if (h->ext != NULL && h->ext->help != NULL)
        (*h->ext->help)(h->ext);
    else
        printText(ARRAY_LEN(text), text);
}
...
static void modehelp(txCtx h) {
    if (h->ext != NULL && h->ext->modehelp != NULL) {
        (*h->ext->modehelp)(h->ext);
        return;
    }
    switch (h->mode) {
        case mode_dump:
...
static void addArgs(txCtx h, const char *filename) {
    int state;
    long i;
    size_t length;
    FILE *fp;
    ...
    length = ftell(fp) + 1;
    h->script.buf = (char *)sMemNew(length);
...
/* Match options. */
static int matchOpts(const void *key, const void *value) {
    return strcmp((char *)key, *(char **)value);
}

/* Return option index from key or txopt_None if not found. */
int getOptionIndex(const char *key, const char *option_array[], int oa_len) {
    const char **optstr =
        (const char **)bsearch(key, option_array, oa_len,
                               sizeof(option_array[0]), matchOpts);
    return (int)((optstr == NULL) ? txopt_None : optstr - option_array + 1);
}

int txGetOptionIndex(const char *key) {
    return getOptionIndex(key, tx_options, ARRAY_LEN(tx_options));
}
...
static void cfrReadFont(txCtx h, long origin, int ttcIndex) {
    ...
    if (h->ext != NULL && h->ext->cfr_ReadFontCB != NULL)
        (*h->ext->cfr_ReadFontCB)(h->ext);
...
/* Process file. */
static void doFile(txCtx h, const char *srcname) {
    long i, srclen;
    char *p;
    struct stat fileStat;
    int statErrNo;
    ...
    /* Set src name */
    if (h->file.sr != NULL) {
        int l = snprintf(h->file.src, sizeof(h->file.src), "%s/", h->file.sr);
        p = &h->file.src[l];
        srclen = sizeof(h->file.src) - l;
    } else {
        p = h->file.src;
        srclen = sizeof(h->file.src);
    }
    if (h->file.sd != NULL)
        snprintf(p, srclen, "%s/%s", h->file.sd, srcname);
    else
        STRCPY_S(p, srclen, srcname);
    ...
    for (i = 0; i < h->fonts.cnt; i++) {
        ...
        if (h->ext != NULL && h->ext->doFileCB != NULL)
            (*h->ext->doFileCB)(h->ext);
...
static int doMultiFileSet(txCtx h, int argc, char *argv[], int i) {
    ...
    h->dst.begset(h);
    for (; i < argc; i++)
        switch (txGetOptionIndex(argv[i])) {
            case txopt_None:
                doFile(h, argv[i]);
                filecnt++;
                break;
...
static void parseArgs(txCtx h, int argc, char *argv[]) {
    ...
    for (i = 0; i < argc; i++) {
        if (h->ext && h->ext->processOption) {
            int ni = (*h->ext->processOption)(h->ext, argc, argv, i);
            if (ni < i)
                goto badarg;
            else if (ni > i) {
                i = ni - 1;
            ...
            // ni == i so fall through
        }
        int argsleft = argc - i - 1;
        arg = argv[i];
        switch (txGetOptionIndex(arg)) {
            case txopt_None:
                /* Not option, assume filename */
                if (argsleft > 0) {
                    char *dstname = argv[i + 1];
                    if (txGetOptionIndex(dstname) == txopt_None) {
                        if (argsleft > 1 && txGetOptionIndex(argv[i + 2]) == txopt_None)
                ...
                break;
            case txopt_dump: /* mode selection options */
                setMode(h, mode_dump);
                break;
            case txopt_ps:
                setMode(h, mode_ps);
            ...
            case txopt_l:
                ...
                break;
            case txopt_0: /* dump/ps/path mode options */
                switch (h->mode) {
                    case mode_dump:
                        h->abf.dump.level = 0;
                        break;
            ...
            case txopt_2:
                ...
                break;
            case txopt_3: /* dump mode options */
                switch (h->mode) {
                    case mode_dump:
                        h->abf.dump.level = 3;
                        break;
            ...
            case txopt_n:
                ...
                break;
            case txopt_no_futile:
                switch (h->mode) {
                    case mode_cff:
                        h->logger->msg(sWARNING, "option -no_futile deprecated (ignored)");
                        break;
                    default:
                        goto wrongmode;
                }
            ...
            case txopt_E:
                ...
                break;
            case txopt__F: /* cff mode options */
                if (h->mode != mode_cff)
                    goto wrongmode;
            ...
            case txopt_t:
                ...
                break;
            case txopt_m: /* Memory failure simulator */
                if (!argsleft)
                    goto noarg;
                else {
                    char *p = argv[++i];
                    ...
                    if (*q != '\0')
                        ...
                        /* Fail on random call */
                        seedtime(h);
                        h->failmem.iFail = randrange(h, cnt - 1);
                    }
                }
                break;
            case txopt_maxs: /* set max number subrs. */
                if (!argsleft)
                    goto noarg;
                else {
                    char *p;
...
void txFree(txCtx h) {
    ...
    if (h->ext != NULL && h->ext->free != NULL)
        (*h->ext->free)(h->ext);
    sMemFree(h->script.buf);
...
/* Main program. */
    ...
    if (argc > 1 && txGetOptionIndex(argv[argc - 2]) == txopt_s) {
    ...
    if (h->failmem.iFail == FAIL_REPORT) {
        fflush(stdout);
        h->logger->log(sINFO, "mem_manage() called %ld times in this run.",
                       h->failmem.iCall);
    }
    return 0;
}
